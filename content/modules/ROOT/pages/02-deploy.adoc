...
[CAUTION]
====
Network changes: This workshop platform is self-hosted, and the cluster and service networks that would ordinarily be used for the hosted cluster pods are used by the hosting cluster.
The YAML configuration of the cluster and service networks for the hosted cluster must be changed.
The network defined below will be used for the _pods_ in the *my-hosted-cluster*, NOT the _nodes_ in the *my-hosted-cluster*.
====
+
...
NOTE: The virtual machines are likely not started yet.  Please be patient.

. Now examine the virtual machines created by this deployment of a hosted cluster.
...
+
NOTE: This usually must be done twice.
...
. A new tab will open and bring you to the node details section on your my-hosted-cluster.
Convenient!
.. Close the browser tab to return to the *my-hosted-cluster - Overview*.

. Examine the node pools.
.. Just in case you're lost - on the *Infrastructure -> Clusters -> my-hosted-cluster* page, scroll down until you see the *Cluster node pools* section.
.. Then click on the *three-dot* menu on the right and select *Manage node pool*.
...
+
image::deploy/deployment_complete.png[link=self, window=blank, width=100%]
