...
. This workshop platform is self-hosted, and the cluster and service networks that would ordinarily be used for the hosted cluster are used by the hosting cluster.
So the YAML configuration of the cluster and service networks for the hosted cluster must be changed.
[loweralpha]
.. Click the *YAML On* switch
.. Change lines 27 throught 31 to match the following IP address ranges.
+
...
+
image::deploy/yaml_on_switch_and_networks.png[link=self, window=blank, width=100%]
